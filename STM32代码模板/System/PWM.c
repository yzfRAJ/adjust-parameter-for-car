...
/*
PWM 输出 PA0  PA1
    频率 72M/(PSC+1)/(ARR+1)=72M/72/50，通过改变psc即可实现不同频率的矩形波
    占空比 CCR/(ARR+1)
*/
void PWM_Init(void)
{
...
void Servo_PWM_Init(void)//舵机用PWM PA8
{
	...
	TIM_TimeBaseInitStruct.TIM_Period = 20000-1;    //计算到20000
	TIM_TimeBaseInitStruct.TIM_Prescaler = 72-1;  //最高频率72MHZ  这里定义 预分频72																					 
